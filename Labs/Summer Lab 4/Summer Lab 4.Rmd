...
transmute("Model 1 Prediction" = predict(lm1, newdata = test.bgg)) %>% 
mutate(test.bgg2$residual_lm1 <- residuals(lm1))


test.bgg2$pred_lm2 <- predict(lm2, newdata = test.bgg)
test.bgg2$residual_lm2 <- residuals(lm2)

test.bgg2$pred_lm3 <- predict(lm3, newdata = test.bgg)
test.bgg2$residual_lm3 <- residuals(lm3)
